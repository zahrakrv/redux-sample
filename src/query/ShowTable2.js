...
import {
  ...
  updateAgeSuccess,
  DeleteAge,
} from '../stateManagement/actions/productThunkActions';
import {
  ApolloClient,
  InMemoryCache,
  useMutation,
...
const ShowTable2 = () => {
  ...
  const products = useSelector((state) => state.productThunkState.items);
  const isLoading = useSelector((state) => state.productThunkState.isLoading);
  const message = useSelector((state) => state.productThunkState.message);
  useEffect(() => {
  ...
  const handleUpdate = (id) => {
    const updatedData = products?.AllAge?.data?.find((age) => age?.id === id);
    if (updatedData) {
    ...
    if (updateId) {
      update({
        ...
        .then((res) => {
          ...
          // refetch();
          // getAllAges();
          dispatch(updateAgeSuccess(res.data));
          dispatch(GetAllProducts());
        })
        .catch((error) => {
          console.log(error);
      ...
      create({
        ...
        .then((res) => {
          ...
          // refetch();
          // getAllAges();
          dispatch(AddAge(res.data.AllAge.data));
          setName('');
          setDescription('');
          setFrom(0);
          setTo(0);
      ...
      <>
        ...
        <div className="flex justify-center">
          <table className="border table-fixed m-3 p-2 w-4/5">
            ...
            <tbody>
              {products?.AllAge?.data?.map((age, index) => {
                return (
                  <tr
